...
export function DraftPostComponent({
  ...
  // const {setActiveDraftPost} = useOutletContext();
  ...
  // const openPost = async (post: ResolvedCoMap<Post>) => console.log("activePost", activePost);
  const openPost = async (post: ResolvedCoMap<Post>) =>
      ...
      <div className="flex gap-2 overflow-x-scroll w-full">
        {post?.images?.map(
          (image) =>
            image &&
            image.imageFile && (
              <div className="relative" key={`container-${image.id}`}>
                <Button
                  variant="destructive"
